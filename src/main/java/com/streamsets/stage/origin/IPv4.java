...
public class IPv4 {
    ...
    public IPv4(String symbolicIP, String netmask) throws NumberFormatException {
        ...
        baseIPnumeric = 0;
        for (String aSt1 : st) {
            int value = Integer.parseInt(aSt1);
            if (value != (value & 0xff)) { throw new NumberFormatException("Invalid IP address: " + symbolicIP); }
        ...
        i = 24;
        netmaskNumeric = 0;
        if (Integer.parseInt(st[0]) < 255) { throw new NumberFormatException( "The first byte of netmask can not be less than 255"); }
        for (String aSt : st) {
            int value = Integer.parseInt(aSt);
            if (value != (value & 0xff)) { throw new NumberFormatException("Invalid netmask address: " + netmask); }
        ...
        for (i = 0; i < 32; i++) {
            if ((netmaskNumeric & ourMaskBitPattern) != 0) { encounteredOne = true; }// the bit is 1
            else { // the bit is 0
                if (encounteredOne) { throw new NumberFormatException("Invalid netmask: " + netmask + " (bit " + (i + 1) + ")"); }
            }
    ...
    public IPv4(String IPinCIDRFormat) throws NumberFormatException {
        ...
        if (st.length != 2) { throw new NumberFormatException("Invalid CIDR format '" + IPinCIDRFormat + "', should be: xx.xx.xx.xx/xx"); }
        String symbolicIP = st[0];
        String symbolicCIDR = st[1];
        Integer numericCIDR = new Integer(symbolicCIDR);
        if (numericCIDR > 32) { throw new NumberFormatException("CIDR can not be greater than 32"); }
        /* IP */
        st = symbolicIP.split("\\.");
        if (st.length != 4) { throw new NumberFormatException("Invalid IP address: " + symbolicIP); }
        int i = 24;
     ...
     *@return
     */
    String getIP() { return convertNumericIpToSymbolic(baseIPnumeric); }
    ...
    public List<String> getAvailableIPs(Integer numberofIPs) {
        ...
        if(31 <= numberOfBits){
            String ip = convertNumericIpToSymbolic(baseIPnumeric);
            result.add(ip);
            return result;
        }
    ...
    public String getWildcardMask() {
        Integer wildcardMask = ~netmaskNumeric;
    ...
    private String getBinary(Integer number) {
        ...
        for (int i = 1; i <= 32; i++) {
            if ((number & ourMaskBitPattern) != 0) { result.insert(0, "1"); } // the bit is 1
            else { result.insert(0, "0"); }// the bit is 0
            if (i % 8 == 0 && i != 32) { result.insert(0, "."); }
            ourMaskBitPattern = ourMaskBitPattern << 1;
        }
        return result.toString();
    }

    public String getNetmaskInBinary() { return getBinary(netmaskNumeric); }
    ...
    public boolean contains(String IPaddress) {
        ...
        if (st.length != 4) { throw new NumberFormatException("Invalid IP address: " + IPaddress); }
        int i = 24;
        for (String aSt : st) {
            int value = Integer.parseInt(aSt);
            if (value != (value & 0xff)) { throw new NumberFormatException("Invalid IP address: " + IPaddress); }
            checkingIP += value << i;
            i -= 8;
        }
    ...
    public boolean contains(IPv4 child) {
        ...
        return (subnetID & this.netmaskNumeric) == (this.baseIPnumeric & this.netmaskNumeric) && (this.netmaskNumeric < subnetMask) && this.baseIPnumeric <= subnetID;
    }

    public boolean validateIPAddress() {
        String IPAddress = getIP();
        return !IPAddress.startsWith("0") && !IPAddress.isEmpty() && "\\A(25[0-5]|2[0-4]\\d|[0-1]?\\d?\\d)(\\.(25[0-5]|2[0-4]\\d|[0-1]?\\d?\\d)){3}\\z".matches(IPAddress);
    }
}